<script setup>
    ...
    const returnRoundColor = () => {
        const colorList = ['#23b7e5','#27c24c','#3a3f51','#fad733','#7266ba','#f05050'];
...
<template>
    <ul class="project-list">
        <li class="project-item" v-if="Array.isArray(content)" v-for="(item,index) in content" :style="{ backgroundColor: returnRoundColor() }">
            <a class="project-link" :href="item.html_url">
        ...
        <NoData v-else :description="'未查询到指定仓库的信息'"/>
    </ul>
</template>